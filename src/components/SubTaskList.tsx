    ...
    <div className="space-y-4">
      ...
      <AnimatePresence initial={false}>
        {subtasks.map((st) => (
          <motion.div
            ...
            exit={{ opacity: 0, x: 10 }}
            className="group flex items-center gap-3 bg-white p-3 rounded-xl border border-slate-100 shadow-sm"
          >
            <button
            ...
            >
              {st.completed && <Check size={14} className="text-white stroke-[3px]" />}
            </button>
            <span className={cn(
      ...
      <form onSubmit={handleAdd} className="relative mt-2">
        <input
          ...
          className="w-full bg-slate-50 border border-slate-100 rounded-xl py-3 pl-4 pr-12 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-purple-100 transition-all"
        />
        <button
          type="submit"
          className="absolute right-2 top-1.5 w-9 h-9 bg-white border border-slate-100 rounded-lg flex items-center justify-center text-slate-400 hover:text-purple-600 shadow-sm"
        >
          <Plus size={18} />
        </button>
      </form>
